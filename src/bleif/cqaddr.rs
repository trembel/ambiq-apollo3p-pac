...
impl R {
    ...
    #[doc = "Bits 2:20 - Bits 19:2 of target byte address for source of CQ (read only). The buffer must be aligned on a word boundary"]
    #[inline(always)]
    pub fn cqaddr(&self) -> CQADDR_R {
        CQADDR_R::new(((self.bits >> 2) & 0x0007_ffff) as u32)
    }
}
impl W {
    #[doc = "Bit 28 - Bit 28 of target byte address for source of CQ (read only). Used to denote Flash (0) or SRAM (1) access"]
