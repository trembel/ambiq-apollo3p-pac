...
impl R {
    #[doc = "Bit 5 - Secondary pre-fetch feature that will cache pre-fetched data across bus wait states (requires DPREFETCH to be set)."]
    #[inline(always)]
    pub fn dprefetch_cache(&self) -> DPREFETCH_CACHE_R {
        DPREFETCH_CACHE_R::new(((self.bits >> 5) & 0x01) != 0)
    }
    #[doc = "Bit 4 - When set, data bus accesses to the SRAM banks will be pre-fetched (normally 2 cycle read access). Use of this mode bit is only recommended if the work flow has a large number of sequential accesses."]
    #[inline(always)]
    pub fn dprefetch(&self) -> DPREFETCH_R {
        DPREFETCH_R::new(((self.bits >> 4) & 0x01) != 0)
    }
    #[doc = "Bit 1 - Secondary pre-fetch feature that will cache pre-fetched data across bus wait states (requires IPREFETCH to be set)."]
    #[inline(always)]
    pub fn iprefetch_cache(&self) -> IPREFETCH_CACHE_R {
        IPREFETCH_CACHE_R::new(((self.bits >> 1) & 0x01) != 0)
    }
    #[doc = "Bit 0 - When set, instruction accesses to the SRAM banks will be pre-fetched (normally 2 cycle read access). Generally, this mode bit should be set for improved performance when executing instructions from SRAM."]
    #[inline(always)]
