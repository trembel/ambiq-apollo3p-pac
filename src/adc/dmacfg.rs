...
pub enum DMAHONSTAT_A {
    #[doc = "0: ADC conversions will continue regardless of DMA status register"]
    DIS = 0,
    #[doc = "1: ADC conversions will not progress if DMAERR or DMACPL bits in DMA status register are set."]
    EN = 1,
}
...
impl<'a> DMAHONSTAT_W<'a> {
    ...
    #[doc = "ADC conversions will continue regardless of DMA status register"]
    #[inline(always)]
    pub fn dis(self) -> &'a mut W {
        self.variant(DMAHONSTAT_A::DIS)
    }
    #[doc = "ADC conversions will not progress if DMAERR or DMACPL bits in DMA status register are set."]
    #[inline(always)]
    pub fn en(self) -> &'a mut W {
